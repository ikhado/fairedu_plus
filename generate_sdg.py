...
def prepare_generate_sdg(dataset_folder, dataset_name, generator, output_file_name, is_run_splitt_file):
    # generator = CONSTANT_CTGAN

    if generator == CONSTANT_LLM:
        path_to_8_files = os.path.join(dataset_folder, dataset_name, 'llm_split_files')
        path_to_no_8_files = os.path.join(dataset_folder, dataset_name, 'llm_no_split_files')
    elif generator == CONSTANT_CTGAN:
        path_to_8_files = os.path.join(dataset_folder, dataset_name, 'ctgan_split_files')
        path_to_no_8_files = os.path.join(dataset_folder, dataset_name, 'ctgan_no_split_files')
    ...
    if is_run_splitt_file:
        ...
        merge_csv_files(folder_path=path_to_8_files, output_file=f"{output_file_name}")
        return os.path.join(path_to_8_files, output_file_name)
    else:
        ...
        # merge all files within path_to_8_files folder into "merged_output.csv" file
        merge_csv_files(folder_path=path_to_no_8_files, output_file=f"{output_file_name}")
        return os.path.join(path_to_no_8_files, output_file_name)


if __name__ == "__main__":
    dataset_name = 'student_dropout'  # student_dropout, student_oulad student_performance, DNU
    prepare_generate_sdg(dataset_name)